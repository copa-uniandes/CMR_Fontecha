...
public class Maintenance {
	 ...
	 */
	public static String OPTMETHOD = NEWTON; //Method selected
	/**
	 * Switch
	 */
	 ...
	 */
	public static String COST_GRAPH=OFF; //To do a graph
	/**
...
//		System.out.println("Esta en el nodo: "+node.getId());
		...
		setPieces(datapieces(node, convexity(node, this.getOpt(), getU(), T), npieces, nouts, getL(), getU()));
	 ...
	 */
	public static double convexity(MNode node, double xopt, double upper, double t){
		...
		do{
			...
			if(costm<=0){
				nh=nm;
				nmaux=nm;
				costmaux=(cost(node,nmaux+TOL)-2*cost(node,nmaux)+cost(node,nmaux-TOL))/(TOL*TOL);
				nm=(nh+nl)/2;
			} else if(costm>0){
				nl=nm;
				nmaux=nm;
...
//		int cual=10;
//		if(node.getId()==cual){
//			System.out.println(nlower+" "+nupper+" "+t);
//		}
...
//			if(node.getId()==cual){
//				System.out.println(pieces[i][0]+" "+pieces[i][1]);
//			}
		}
...
//			}
		...
		for(int i=(npieces-nout);i<npieces;i++){
			if(d(node,nupper+((i-npieces+nout+1)*step))<pieces[i-1][0]){
...
//				System.out.println("Esta haciendo newton pero entra a fuerza bruta");
				x1=force(node,1,t);
				return x1;
			}
	...
	static double costgraphpreventive(MNode node,double nx){
		...
		double preventivecost=CPM*(1-ans1)/((nx+TPM)*(1-ans1)+(nx+TCM)*ans1);
	...
	static double costgraphcorrective(MNode node,double nx){
		...
		double ans1= node.getD().cdf(nx);
		double m=CostFunction.calcMdeltaLib(node.getD(),0,nx);
		double correctivecost=(CCM)*ans1/((nx+TPM)*(1-ans1)+(nx+TCM)*ans1);
	...
	static double costgraphwaiting(MNode node, double nx){
		...
		double TCM =node.getTcm();
		double ans1= node.getD().cdf(nx);
		double pot=node.getPot();
		double m=CostFunction.calcMdeltaLib(node.getD(),0,nx);
		double waitingcost=(CW*Math.pow((nx-m),pot))*ans1/((nx+TPM)*(1-ans1)+(nx+TCM)*ans1);
		return waitingcost;	
	}
	/**
	...
	public static double cost(MNode node,double nx){
		...
		double pot=node.getPot();
		double m = CostFunction.calcMdeltaLib(node.getD(), 0, nx);
		double cost1=CPM*(1-ans1)/((nx+TPM)*(1-ans1)+(nx+TCM)*ans1);//m+W
		double cost2=(CCM+CW*Math.pow((nx-m),pot))*ans1/((nx+TPM)*(1-ans1)+(nx+TCM)*ans1);
	...
	static double expectedcost(MNode node, double nx){
		...
		double cost2=(CCM+CW*Math.pow((nx-m),pot))*ans1;
		double totalcost=cost1+cost2;
		return totalcost;	
	}
	/**
